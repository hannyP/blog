...
def get_all_posts():
    ...
    return render_template("index.html", all_posts=posts,log_in=current_user)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        hased = generate_password_hash(form.password.data, method='pbkdf2:sha256', salt_length=8)
    ...
    return render_template("register.html",form=form,log_in=current_user)
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            ...
            login_user(user)
            return redirect(url_for('get_all_posts'))
    return render_template("login.html",form=form,log_in=current_user)
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
    return render_template("post.html", post=requested_post,log_in=current_user,form=form)
...
def edit_post(post_id):
    ...
    return render_template("make-post.html", form=edit_form,log_in=current_user)
